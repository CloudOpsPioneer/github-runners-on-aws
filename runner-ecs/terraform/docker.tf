...
resource "docker_image" "image" {
  name = "${aws_ecr_repository.gh_runner.repository_url}:latest"
  build {
...
resource "null_resource" "folder_change_trigger" {
  ...
  provisioner "local-exec" {
    command = <<EOT
      aws ecr get-login-password --region ${var.region} | docker login --username AWS --password-stdin ${aws_ecr_repository.gh_runner.repository_url}
      docker push ${aws_ecr_repository.gh_runner.repository_url}:latest
    EOT
  }
